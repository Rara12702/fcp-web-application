...
func (ct *categoryAPI) UpdateCategory(c *gin.Context) {
	...
	categoriesID, err := strconv.Atoi(categoryID)
	if err != nil {
	...
	err = ct.categoryService.Update(categoriesID, updatedCategory)
	if err != nil {
		c.JSON(http.StatusInternalServerError, model.ErrorResponse{Error: err.Error()})
		return
	}

	// Menampilkan status OK (200) jika kategori berhasil diperbarui
